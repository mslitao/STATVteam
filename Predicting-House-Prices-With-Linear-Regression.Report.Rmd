...
evaluate_model = function(fit, data, response_log = FALSE){
  ...
  c(rmse, rmse_log, get_loocv_rmse(fit), r2, get_adj_r2(fit), get_bp_decision(fit, alpha), get_sw_decision(fit, alpha), get_num_params(fit))
}
```
...
df = data.frame(Measure = c("RMSE", "RMSE Logged", "loocv_RMSE", "R2", "Adj. R2", "bp_decision", "sw_decision", "num_params"),
...
```{r}
data_with_dummy_selected = cbind(data_selected, data[, c("CentralAir", "KitchenQual", "SaleType")])
...
Fit the model.
```{r}
fit = lm(log(SalePrice) ~ . + (OverallQual + GrLivArea + GarageCars + TotalBsmtSF + X1stFlrSF  + MasVnrArea + Fireplaces+ YearBuilt ) ^2 + log(LotArea)  + log(GrLivArea)+log(X1stFlrSF)+log(GarageArea) , data = training_with_dummy)

df = data.frame(Measure = c("RMSE", "RMSE Logged", "loocv_RMSE", "R2", "Adj. R2", "bp_decision", "sw_decision", "num_params"),
                Train = evaluate_model(fit, training_with_dummy, TRUE),
                Test = evaluate_model(fit, testing_with_dummy, TRUE))
...
df = data.frame(Measure = c("RMSE", "RMSE Logged", "loocv_RMSE", "R2", "Adj. R2", "bp_decision", "sw_decision", "num_params"),
                Train = evaluate_model(fit_aic, training_with_dummy, TRUE),
                Test = evaluate_model(fit_aic, testing_with_dummy, TRUE))
