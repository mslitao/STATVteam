...
There a 79 predictors in the original dataset, and some of them describe the similar property of a house and are linearly related. We want to find out the colinearity relationship with of each predictor with others.
...
We split the predictors into numerail and factor. In this section, we will investigate the properties of factor predictors and find out proper dummy variable for our model.
...
The above plots are our selected dummy variables. They don't have too many levels, and contain information that numerical variables don't have. They have relatively balanced amout, and obvious difference of house price among levels.
...
kable(df)
```
It's clear that the more complex model start to overfitting which is a bad model.
...
- Since the `test.csv` data set do not contain the Sale price, we can double check the result. 
- However, with using the final model `fit_final`, we are 95% confidence that the mean Sale price is `r  data_test_1_confidence["fit"]` and within `r  data_test_1_confidence["lwr"]` and `r  data_test_1_confidence["upr"]`
