...
evaluate_model = function(fit, data, response_log = FALSE){
  ...
  c(rmse, rmse_log, get_loocv_rmse(fit), r2, get_adj_r2(fit), 
    get_bp_decision(fit, alpha), bptest(fit)$p.value,
    get_sw_decision(fit, alpha), shapiro.test(resid(fit))$p.value,
    get_num_params(fit))
}
...
df = data.frame(Measure = evaluate_model_name,
                fit_Train = evaluate_model(fit, training_with_dummy, TRUE),
                fit_Test = evaluate_model(fit, testing_with_dummy, TRUE),
                fit_aic_Train = evaluate_model(fit_aic, training_with_dummy, TRUE),
                fit_aic_Test = evaluate_model(fit_aic, testing_with_dummy, TRUE)
                )
...
```{r}
df = data.frame(Measure = evaluate_model_name,
                fit_final_Train = evaluate_model(fit, training_with_dummy, TRUE),
                fit_final_Test = evaluate_model(fit, testing_with_dummy, TRUE))
