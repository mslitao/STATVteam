...
for(i in 1:n){
  ...
  correlations_log_na[i] = cor(log(train[,numericFeatures[i]]), train[,"SalePrice"])
  correlations_na_log[i] = cor(train[,numericFeatures[i]], log(train[,"SalePrice"]))
  correlations_log_log[i] = cor(log(train[,numericFeatures[i]]), log(train[,"SalePrice"]))
}
...
```{r}
fit = lm(log(SalePrice) ~ . + YearBuilt:OverallQual + YearRemodAdd:OverallQual + OverallQual:TotalBsmtSF + OverallQual:GrLivArea + OverallQual:FullBath + log(LotArea)  + log(GrLivArea)+log(X1stFlrSF)+log(GarageArea), data = data)

cal_rmse(log(data$SalePrice), predict(fit, newdata = data))
...
cal_rmse(log(data$SalePrice), predict(fit_aic, newdata = data))
```
...
cal_rmse(log(data$SalePrice), predict(fit_bic, newdata = data))
```
...
```{r}
fit = lm(log(SalePrice) ~ .^2, data = data)
